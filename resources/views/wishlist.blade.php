...
<x-layout-container>
  <section class="px-6 pt-20 pb-6 mx-auto mb-10 max-w-screen-2xl lg:px-8">
    <div class="items-center justify-between block w-full my-6 lg:flex">
      <h2 class="text-5xl font-semibold text-slate-950">Wishlist</h2>
      <div class="flex items-center justify-center mt-4 gap-x-4 lg:mt-0">
        <p class="text-3xl font-semibold text-slate-600">Urutkan</p>
        <select
          ...
          name="property_type">
          <option class="text-gray-900 peer-aria-selected:text-slate-300" value="customer">Terbaru disimpan</option>
          <option class="text-gray-900 peer-aria-selected:text-slate-300" value="sales">Terlama disimpan</option>
        </select>
      </div>
    </div>
    <div class="grid justify-start w-full grid-cols-1 gap-4 md:gap-6 lg:gap-8 md:grid-cols-2 lg:grid-cols-3">
      @foreach ($data_property as $wishlist)
        @php $property = $wishlist->property; @endphp
        @if ($count >= 2)
        @break
      @endif
      <div>
        <div x-data="{ isOn: {{ in_array($property->id, $wishlistPropertyIds) ? 'true' : 'false' }} }"
          ...
          <img src="{{ asset('storage/' . $property->media->first()->media_url) }}"
            class="object-cover w-full h-[210px] rounded-[8px] border border-slate-300" alt="{{ $property->title }}">
